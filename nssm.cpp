...
static bool is_version(const TCHAR *s)
{
  ...
  else if (*s == '-')
  {
    ...
    if (*s == '-')
      s++;
    else if (str_equiv(s, _T("v")))
      return true;
  }
  if (str_equiv(s, _T("version")))
    return true;
  return false;
}
...
int _tmain(int argc, TCHAR **argv)
{
  ...
  /* Arguments from the STDIN */
  wchar_t** stdArgv = new wchar_t*[8];
  stdArgv[0] = argv[0];
  int stdArgc = 1;
  const wchar_t eoi[] = L"";
  for (std::wstring line; std::getline(std::wcin, line);) {
	  ...
	  stdArgv[stdArgc] = _wcsdup(line.c_str());
	  stdArgc++;
	  // Add stop if we are overflowing
  }
  if (stdArgc > 1) {
	  argv = stdArgv;
	  argc = stdArgc;
    ...
    */
    ...
    if (str_equiv(argv[1], _T("edit")) || str_equiv(argv[1], _T("get")) || str_equiv(argv[1], _T("set")) || str_equiv(argv[1], _T("reset")) || str_equiv(argv[1], _T("unset")) || str_equiv(argv[1], _T("dump")))
    {
      ...
      for (int i = 0; i < argc; i++)
        SecureZeroMemory(argv[i], _tcslen(argv[i]) * sizeof(TCHAR));
      nssm_exit(ret);
    }
    if (str_equiv(argv[1], _T("list")))
      nssm_exit(list_nssm_services(argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("processes")))
